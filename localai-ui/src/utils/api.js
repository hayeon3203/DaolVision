...
// Task 6.3: :8700 anim-agent 게이트웨이 배선 (T2I/I2V 단발샷/I2I/독립 TTS/Agent).
// LocalAI 자체 추론 백엔드(모델 레지스트리)는 쓰지 않는다 — 별도 오리진(:8700)의
// 고정 로컬 파이프라인을 직접 호출한다. 같은 GB10 호스트, 다른 포트라 크로스오리진 —
// 게이트웨이가 CORS를 허용한다(langgraph/api.py).
export const GATEWAY_BASE = `http://${window.location.hostname}:8700`

async function gatewayFetchJSON(path, options = {}) {
  const response = await fetch(`${GATEWAY_BASE}${path}`, options)
  return handleResponse(response)
}

async function gatewayFetchBlob(path, options = {}) {
  const response = await fetch(`${GATEWAY_BASE}${path}`, options)
  if (!response.ok) {
...
export const gatewayApi = {
  // 독립 T2I 카테고리 — POST /t2i, base64 PNG 반환
  t2i: (body) => gatewayFetchJSON('/t2i', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  }),
  ...
  i2i: (style, imageFile, seed) => {
    ...
    form.append('image', imageFile)
    if (seed !== '' && seed !== undefined && seed !== null) form.append('seed', seed)
    return gatewayFetchJSON('/i2i', { method: 'POST', body: form })
  },
  // 독립 TTS 카테고리(Chatterbox V3 zero-shot clone) — POST /tts/clone (multipart), WAV blob 반환
  ttsClone: (text, referenceWavFile) => {
    const form = new FormData()
