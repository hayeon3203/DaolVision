...
- 라이브러리 환경: torch 2.12.1+cu130, transformers 5.12.1
...
> 실측 아님. "미확인"은 출처에서 못 찾은 값 — 추정치로 채우지 않음. Peak과 Load를 분리해서 보고한 소스가 드물어 Peak이 전반적으로 가장 약한 데이터.

### LLM (Multimodal)
FP8이나 INT4로 양자화(Quantization)하여 가중치 용량을 절반 이하로 줄이는 과정을 선행이 필수
