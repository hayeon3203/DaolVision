...
| Wan2.2-Animate 저장소 | `/home/admin/video_generator/Wan2.2-Animate` | 미실측(대형) | `animate_server.py`의 `WAN_ANIMATE_REPO` 기본값이 가리키는 참조코드/체크포인트 소스 | env `WAN_ANIMATE_REPO`로 재지정 가능. 이번 태스크에서는 미이동 |
...
|---|---|---|---|---|
| Wan2.2-TI2V-5B (T2V/I2V) | :8500 | `hunyuan_server/server.py` | `hunyuan_server/deploy/wan.service` | 수동 실행 중(systemd 비활성) |
| FLUX.1-schnell (T2I) | :8501 | `hunyuan_server/flux_server.py` | `hunyuan_server/deploy/flux.service` | 수동 실행 중(systemd 비활성) |
| Wan2.2-Animate-14B | :8600 | `hunyuan_server/animate_server.py` | `hunyuan_server/deploy/wan-animate.service` | 미사용(LangGraph가 안 씀, STATE.md 3.6 참고) |

deploy unit 3종은 `WorkingDirectory`/`ExecStart`를 `/home/admin/DaolVision/hunyuan_server`로
갱신했다(`sed` 일괄 치환). `~/.config/systemd/user/{wan,flux,wan-animate}.service`
설치본도 동일하게 갱신했으나, 셋 다 이전부터 `disabled`/`inactive`였고 이번에도
...
`hunyuan_server/weights/Z-Image-Turbo`(31GB, video_generator에 잔존)는 폐기된
