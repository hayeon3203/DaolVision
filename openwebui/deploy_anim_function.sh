#!/usr/bin/env bash
# Deploy openwebui_anim_function.py (repo -> Open WebUI DB function.content) and reload.
#
...
# This is the single source of truth for the "Animation Video Agent" model.
#
# 사용법: openwebui_anim_function.py 수정 후  ./deploy_anim_function.sh
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/openwebui_anim_function.py"
CONTAINER="open-webui"
FN_ID="animation_video_agent"
PY="${PY:-python3}"
