...
# 5씬 무드등 광고. 인물 씬(1~4)은 손동작 어휘 금지(집어/들고/쥐고/마시), 제품 명사 금지
# — 걸리면 그립/조립 경로로 새거나 배경이 램프를 또 그린다. 마지막 씬만 제품 명사 +
# 인물 없이. 각 문장에 **장소 단어**를 넣어 setting 환각을 막는다(서재/거실/침실).
SCENARIO = (
    "저녁 거실 소파에 온 가족이 나란히 앉아 도란도란 이야기하며 편안히 쉰다. "
    "어두운 서재 책상 앞에 앉은 20대 남자가 노트북으로 작업하다 기지개를 켜며 창밖을 본다. "
    "은은한 조명이 감도는 서재 책상 앞에 앉은 40대 남자가 책을 천천히 읽으며 넘긴다. "
    "어두운 침실 침대 가장자리에 걸터앉은 30대 여자가 편안히 휴식한다. "
