...
잘못된 방향으로 오래 진행하면 손해가 크기 때문에, 사람이 중간 결과를 확인하고 승인/수정/재생성을
지시하는 **휴먼 승인 게이트 3곳**을 일부러 남겨둔 "사람이 감독하는 자동화"입니다.

특징:

- **완전 로컬**: LLM(Ollama), 영상 생성(Wan2.2/Wan2.1), 편집(ffmpeg) 전부 한 대에서. 외부 API 의존 없음.
- **중단 후 재개**: 상태를 SQLite 체크포인트로 저장하므로, 몇 시간 뒤에 승인해도 중단된 지점부터 이어서 진행합니다.
- **얼굴 일관성**: 참조 이미지를 주면 해당 인물 씬은 Stand-In(14B) 경로로 라우팅되어 얼굴이 유지됩니다.
- **채팅으로 제어**: Open WebUI에서 자연어로 지시합니다 (예: `승인`, `재생성 2,4`, `씬3에 2번째 사진 넣어줘`).
...
이 에이전트는 전체 과정을 조율하는 역할만 담당하고, 실제 생성 작업은 아래 서비스들이 수행합니다. 네 가지 서비스가 모두 실행되어 있어야 전체 파이프라인이 동작합니다.
...
curl http://127.0.0.1:8504/health      # Chatterbox V3 사용자 음성 복제
...
- 재생성 여부는 **항상 사람이 결정**합니다. GPU를 무한정 소모하지 않도록 AI가 스스로 재생성 루프를 반복하지 않습니다.
- 승인 게이트는 3곳으로 고정했습니다. 게이트가 더 많으면 승인 부담이 커지고, 더 적으면 재작업 비용이 커지기 때문입니다.
...
| `POST /tts/narration` | Chatterbox V3 고정 CC0 한국어 화자 영상 나레이션 |
| `POST /tts/clone` | 참조 WAV 기반 Chatterbox V3 사용자 음성 생성 (multipart) |
...
- **LLM 환각 방어**: 씬 분할 LLM이 존재하지 않는 참조 이미지 파일명을 만들어 내면 해당 씬을 자동으로 T2V 경로로 전환합니다.

## GB10 최적화 (성능 튜닝 이력)

GB10은 CPU/GPU가 119GB 통합메모리를 공유합니다. VRAM을 절약하기 위한 오프로드 기법은
같은 DRAM 안에서 복사만 반복하게 되어 아무 이득이 없으므로, 전부 제거하고 모델을 상주시켰습니다.
...
모델 로딩 시간(1회당 약 2분)은 모델 상주화와 ComfyUI의 systemd 서비스화를 통해 사라졌습니다.

⚠️ 이 스택에서 확인된 금지 조합:
- `sageattn_3`(fp4): GB10에서 커널 dtype 에러가 발생합니다
- `fp8_e4m3fn_fast`: 비병합 LoRA(SetLoRAs) 구조와 호환되지 않습니다
- **sageattn + torch.compile**: 실행할 때마다 재컴파일이 반복되어 30분 이상 멈추므로 compile을 반드시 제거해야 합니다

## 기술 결정: 왜 OpenShell이 아니라 LangGraph인가

초기 프로토타입(`orchestrator/`)은 OpenShell을 기반으로 했으나, 다음과 같은 이유로 부적합하다고 판단해 LangGraph로 전환했습니다.
...
요약하면, OpenShell은 작업을 실행하는 기능만 제공하지만 이 파이프라인의 본질은 **중단했다가 재개하는 승인 워크플로우와 상태 관리**이며,
LangGraph는 그 기능을 프레임워크 차원에서 제공합니다. 상세 비교는 `BRIEFING.md` 참조.

## Open WebUI 연동

`openwebui_anim_function.py`를 Open WebUI Function으로 등록하면 채팅에서 전 과정을 제어할 수 있습니다.
승인 과정이 여러 턴에 걸쳐 진행되므로, job과 체크포인트 정보를 어시스턴트 메시지 끝의 HTML 주석 마커에 담아 유지합니다.
