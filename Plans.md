...
| 6.15 | Agent UI에 M2(이미지 설명으로 생성) 입력 분기 없음 + 2-3 체크포인트 자동승인이라 생성 이미지 리뷰 불가 (GatewayAgent.jsx — script_text/ref_images 파일첨부만 있고 image_request 미배선, `autoDecisionFor`가 `2-3` 체크포인트 무조건 자동승인) [tdd:skip:ui-component] | "사진 첨부" vs "이미지 설명으로 생성" 입력 분기 UI + 2-3용 AgentScenePreview류 프리뷰 컴포넌트(생성 이미지 확인 후 승인/재생성) | - | 6.6, 6.8 | cc:완료 [d2bfc9b] | - |
...
| 6.18 | scene.mood가 무인물 씬에서 노이즈로 작용할 수 있다는 의혹 제기 및 조사 (2026-08-03 사용자 지적) — `_scene_prompt_system`은 subject_type=nonhuman/none 씬에서 인물묘사 지시를 통째로 빼므로 mood의 EXPRESSION 채널(nodes.py:851)이 죽고 `_mood_to_lighting` lighting 폴백만 남음. 실측: job 003cb843 scene2(우주선 호버링, 인물 0), bfbda793 scene2(전기 아크+우주선, 인물 0) 둘 다 mood=excited인데 실제로 "흥분"할 주체가 없고 "bright vivid lighting"류 문구만 입혀짐. human 씬에서도 mood-lighting 불일치 관측(job 1ec2b072 scene2: mood=excited인데 최종 lighting=low-key dim, cool ambient glow) — 씬분할 단계 mood 판정과 rewrite 단계 lighting 생성이 서로 검증 없이 독립적으로 이뤄지는 게 근본 원인으로 추정. mood 삭제 여부/부분수정 여부 미결정 | 삭제/수정 여부 결정 후 재작성 필요 — 현재는 조사기록만 | - | 6.17 | cc:TODO | - |
| 6.19 | 셀카→3D 클레이 피규어(360도 조작) 파이프라인 스파이크 — 얼굴 왕대갈(줌인 드리프트) 원인 진단 + ControlNet 픽스 (2026-08-05) [tdd:skip:diagnostic-fix]. **라이선스 조사**: Arc2Face·IP-Adapter FaceID 둘 다 ArcFace/InsightFace 임베딩 의존 → InsightFace 비상업 연구용 라이선스 동일 적용(국적 무관, "중국산이라 우려"는 틀린 프레임). 이 프로젝트는 GB10 데모 목적 비상업이라 라이선스 문제 자체는 없음 — 다만 왕대갈 원인이 모델 선택이 아니라 확인돼 FaceID 계열 모델 스왑 보류. **왕대갈 원인**: `_build_flux_kontext_graph`(tools.py)가 ControlNet 없이 순수 텍스트 프롬프트(“Keep the same facial features, pose, and composition”)로만 구도를 고정하고 있어 Flux Kontext dev 특유의 줌인 드리프트를 못 잡음(크롭·FaceID 문제 아님, 프롬프트 강화 1번안은 사용자 확인상 무효). **적용한 수정**: ComfyUI Canny(내장, 신규 의존성 없음) + Shakker-Labs FLUX.1-dev-ControlNet-Union-Pro-2.0(4.3GB, `/home/admin/video_generator/ComfyUI/models/controlnet/flux1-dev-controlnet-union-pro-2.0.safetensors`) 노드를 그래프에 추가, `AGENT_FLUX_CONTROLNET_UNION`/`AGENT_FLUX_CONTROLNET_STRENGTH`(기본 0.6) 환경변수로 노출. 건호군.jpg + claymation 스타일로 실측: 왕대갈(구도 드리프트) 해결 확인, 단 strength=0.6에서 clay 재질감이 과도하게 눌림(구도 고정↔스타일 자유도 트레이드오프, strength 하향 재실험 필요). **미해결**: 원래 목적이 전신 3D 피규어인데 이 파이프라인은 입력이 클로즈업 셀카면 출력도 클로즈업 — ControlNet은 입력 구도를 "고정"할 뿐 확장 안 함. 전신 생성은 별도 접근(전신 포즈 템플릿 ControlNet, outpainting, 또는 별도 T2I 전신 생성 후 얼굴만 IP-Adapter/FaceID 이식) 필요, 미착수. **후속(2026-08-06, `DaolVision-avatar3d-spike` 워크트리 `feat/avatar3d-webcam-spike` 브랜치, 커밋 2a55ef4 — main 미병합)**: 전신 포즈 템플릿 ControlNet 노선 채택·구현. `man_proportation.png`(7.5등신 스톡사진) 기반 canny 템플릿을 만들되 얼굴~목 위 영역을 통째로 블랙아웃해서 ControlNet이 몸통 비율/포즈만 강제하고 얼굴 identity는 ReferenceLatent(셀카)가 전담하게 분리 — 앞서 시도한 얼굴사진 collage+blur 트릭(`probe_avatar_template_live.py`)은 identity 픽셀이 그대로 섞여 들어가 실패(왕대갈 잔존, 성별 드리프트)했던 것과 달리 ControlNet은 별도 conditioning 채널이라 구조적으로 분리됨. 건호군.jpg 실측으로 확인. 이어서 노선 전환 — 사용자 지시로 "왕대갈=버그" 대신 "왕대갈=목표"인 1.75등신 챠비 클레이메이션으로 스파이크 확장: 사진 레퍼런스 없이 도형으로 그린 슬림 실루엣 템플릿 + `generate_avatar_source(body_type="chibi")` 추가(`AVATAR_SOURCE_PROMPT`/`CHIBI_SOURCE_PROMPT` 분기, steps 12→20). 1차 시도(2.5등신, 두꺼운 팔다리)가 "chubby하고 딴사람 같다" 피드백 → 폭 슬림화 + 프롬프트에서 애니메 속눈썹/큰눈 유발 문구 제거로 재조정, 남성 identity 보존 확인. `avatar_chibi_1_75head_v3.png` 결과물이 클레이/토이피규어 재질까지 프롬프트 자체에 포함돼 있어 별도 claymation 스타일 패스 불필요. **GPU OOM 노트**: 실측 중 이 프로젝트와 무관한 `foundry-hybrid-agent`의 vLLM(EXAONE-3.0-7.8B, gpu-memory-utilization 0.2→27.4GB 상주)이 동시 상주해 ComfyUI(Flux Kontext+ControlNet, ~21GB) 로드 중 2회 OOM 크래시 재현 — vLLM 프로세스 SIGTERM 후 정상 실측(현재 vLLM 중지 상태, 재기동 여부 미정). **남은 것**: 이 스파이크는 `feat/avatar3d-webcam-spike` 브랜치 전용이라 main 미병합 — 병합 여부/시점 미정. TripoSR 3D 변환(`generate_avatar3d`)은 챠비 경로로 아직 미실측. 3D 리깅/애니메이션(360도 조작 이후 "움직이는" 단계)은 별도 접근 필요, 미착수 | 3D 리깅/애니메이션 경로 조사 + main 병합 여부 결정 후 재작성 | - | 6.1 | cc:WIP | - |
| 6.22 | Agent 입력을 "얼굴 생성 + 제품 첨부" 동시 조합으로 확장 (지금은 배타 — 제품을 첨부하는 순간 `ref_images`가 차서 `graph.py:19` `_entry_router`의 `and not state.get("ref_images")` 조건에 걸려 M2 이미지 생성 분기가 아예 안 걸리고 첨부 이미지만 쓰는 기존 경로로 빠짐. 프론트도 `GatewayAgent.jsx:36` `inputMode`가 upload/describe/noref 3택1이라 조합 자체가 없음) [tdd:required] | ① `graph.py:19` 라우터에서 `and not ref_images` 제거 ② `nodes.py:158-169` 승인 게이트가 `ref_images`를 새 리스트로 덮어쓰는 걸 기존 값에 append로 변경 + 생성분 파일명을 `img_{i}.png`→`gen_{i}.png`로 개명(업로드분이 `api.py:126`에서 같은 이름을 쓰므로 충돌) ③ `GatewayAgent.jsx:85-92` inputMode 3택1 해제(`api.js:710`은 이미 셋 다 전송하므로 :8700 API 변경 없음). 참조 2장(human+nonhuman) job이 씬분할까지 완주 | cd langgraph && ./.venv/bin/python tests/test_gen_plus_upload_refs.py | 6.15 | cc:TODO | - |
| 6.23 | 제품 씬 A노선(첫 프레임 조립) 프로덕션 이식 — 6.22로 흐름을 연결해도 제품 씬은 `character_ref`→`SUBJECT_REF`(Wan `i2v_14b.json`, `nodes.py:1240`)로 가서 음료 광고 스파이크 산출물과 달라짐. 스파이크가 쓴 "T2I 씬배경 → 제품 픽셀 Pillow 합성 → Kontext 재통합(blur 10/4) → LTX I2V" 조립 단계를 하는 노드가 프로덕션 그래프에 없고, 설계문서가 subject_ref를 "참조에 없는 새 요소를 못 그림"으로 판정해 A노선을 만든 것이라 현 경로는 그 약점으로 회귀함. **범위 결정(2026-08-13 사용자)**: 합성 비율(width/center_x/bottom_y)은 조정 UI를 붙이지 않고 스파이크에서 발굴한 best-case 값을 그대로 고정한다 — 배경 시드까지 고정하면 재현이 보장되기 때문. 일반 시나리오에서는 배경이 새로 생성돼 비율이 깨지는 게 실측돼 있으나(씬2에서 의상 단어 하나 바꾸자 벤치 구조가 통째로 변함, 씬3a는 center_x 6% 이동만으로 붕괴), 이 스파이크 재현이 목적이라 YAGNI로 처리. 실제로 깨지는 걸 본 뒤에 조정 UI 검토. **6.22 실행 결과(LLM이 실제로 어떻게 씬을 나누고 뭘 매칭하는지)를 보고 노드 입력 형태를 정한 뒤 착수** | 설계 후 재작성 | - | 6.22 | cc:TODO | - |
| 6.24 | 씬 프롬프트 품질 보장 — **few-shot(C) + 검증기 재요청(A)** 조합. 템플릿 강제(LLM을 문자열 포매터로 격하)는 시나리오가 조금만 달라져도 안 맞고 LLM 해석력을 못 쓴다는 판단으로 기각(2026-08-13 사용자). **C**: `_scene_prompt_system`에 스파이크 성공/실패 프롬프트를 대조 예시로 넣어 문체를 유도한다 — 좋은 예 `the man runs from far in the distance straight toward the camera, growing larger and closer with each stride, camera stays low and steady`, 나쁜 예 `a solitary player sprinting powerfully, his right arm extended fully forward, gripping the textured basketball with determined force`(자세·질감만 있고 어디서 어디로 가는지가 없음). **A**: 결과를 결정론적으로 검사해 실패 항목만 짚어 1회 재요청 — ① 이동 궤적 어휘(`toward`/`across`/`from … to`/`growing larger`/`approaching`) 유무 ② 조립 배경 프롬프트에 제품 명사 부재 ③ 실외/실내가 setting과 일치. `node_split_scenes`가 씬 개수 오류·문장 절단에 이미 쓰는 교정 재요청 패턴을 그대로 재사용한다. **푸는 것**: 슬로우모션(job e9059c29 씬2·4가 2초간 제자리 — LLM이 궤적 대신 자세·질감을 서술해 LTX가 정지 화면으로 해석), 배경 T2I의 제품 중복 생성, 실내 마루 오해(`polished wooden surface`). **부족하면 D로 승격**: LLM이 문장 대신 구조화 JSON(`subject`/`action`/`start`/`end`/`camera`/`surface`)을 내고 코드가 조립 — LLM은 "무엇을", 코드는 "어떻게 쓸지"를 담당. 오늘 매칭·장소·인물 판정에 통한 처방("판단은 LLM, 형식은 코드")의 정석이고 템플릿화의 상위 호환 | 설계 후 재작성 | - | 6.23 | cc:TODO | - |
| 6.25 | 조립 씬 **인물 일관성** 구조 결정 — 1·2단계로 안 풀리는 유일한 항목. 조립 배경을 T2I로 새로 그리는 한 인물이 매번 다른 사람이 된다(job e9059c29 씬2: 곱슬머리 다른 인종). 후보 ① Kontext로 인물 정본 이식 ② 일관성 포기하고 "여러 사람이 제품을 쓰는" 광고로 전환 ③ 제품 씬에 얼굴을 안 넣는 구도(손·뒷모습·제품 단독). **스파이크 자체가 ①+③ 혼합이었다** — 씬2는 인물이 원거리 실루엣이라 얼굴이 안 보였고(T2I로 충분), 얼굴이 필요한 씬3a만 Kontext 이식 경로였다. 두 경로 모두 코드에 이미 있으므로 시나리오 문장을 그 구도로 유도하는 게 실질 작업 | 설계 후 재작성 | - | 6.24 | cc:TODO | - |
...
| 7.6 | T2V 단발샷 카테고리 스파이크 (Cosmos-Predict2-2B/Cosmos 3 Nano, 추후·낮은 우선순위) [tdd:skip:benchmark] | Cosmos-Predict2-2B(2B)와 Cosmos 3 Nano(16B, 2026-06-01 출시, 8B dense 트랜스포머 기반. bf16 weight만 ~32GB로 GB10 119GB엔 수치상 들어가나 ARM64+Blackwell 커널 호환 미검증. Qwen3-VL 8B 아키텍처 기반이라 R10 국적 배점 계보상 애매하지만 identity 불필요한 순수 T2V 용도라 무방) 둘 다 단발샷 생성 실측·비교, model-selection-t2v.md 신설·기록 (2026-08-02 사용자 지시로 Cosmos-Predict2-2B 비교 생략, Cosmos3-Nano 단독 실측·채택. ARM64+Blackwell 커널 호환 확인됨 — warm 61.5s/640x480/49프레임, GB10 통합메모리에선 offload가 순손해로 확인돼 기본 꺼짐. LocalAI UI I2V 단발샷 카테고리를 T2V로 완전 대체) | cd langgraph && ./.venv/bin/python tests/test_t2v_oneshot.py | 3.8 | cc:완료 [456b006] | - |
...
| 8.2.1 | 모델 서버 OpenShell 샌드박스 격리 (Ollama:11434/ComfyUI:8188/T2I:8501/Kokoro:8503/Chatterbox:8504/Cosmos3Nano:8505 총 6종 — Kokoro는 onnxruntime CPU판이라 `--gpu` 없이, 나머지 5종은 `--gpu` 샌드박스 + `forward`로 기존 localhost 포트 유지, tools.py 코드 변경 없음). **웨이트·venv·코드는 호스트에 그대로 두고 API 서버 프로세스만 격리** — openshell **0.0.96** docker 드라이버가 호스트 bind mount 를 정식 지원한다. 게이트웨이에 `[openshell.drivers.docker] enable_bind_mounts = true` 를 열고(`~/.config/openshell/gateway.toml` + `~/.config/openshell/gateway.env` 의 `OPENSHELL_GATEWAY_CONFIG`, systemd unit 은 무수정 — `EnvironmentFile=-%E/openshell/gateway.env` 를 이미 읽음), 샌드박스마다 `--driver-config-json '{"docker":{"mounts":[{"type":"bind","source":...,"target":...,"read_only":true}]}}'` 로 호스트 경로를 **동일 절대경로**에 물린다(venv 의 shebang·pyvenv.cfg 가 절대경로라 위치를 바꾸면 인터프리터 사슬이 끊김). 스키마는 CLI 도움말에 없어 게이트웨이 바이너리에서 확인: `DockerDriverMountConfig` = bind/volume/image/tmpfs, 필드 `source`/`target`/`read_only`/`subpath`/`options`/`size_bytes`/`selinux_label`. **(2026-08-03 0.0.26 → 0.0.96 업그레이드로 GPU 차단 2건 모두 해소 — 실측)** ① cuInit: `openshell sandbox exec`(supervisor 경유)가 0.0.26 에서 `rc=304 (cudaErrorOperatingSystem)` 였으나 0.0.96 에선 `rc=0`, `cuCtxCreate`/`cuMemAlloc 256MB` 까지 성공, `nvidia-smi` 도 정상(드라이버 580.142 로 호스트와 일치 — 0.0.26 공식 GPU 이미지의 NVML 580.159 불일치도 소멸). ② GPU 1개 제약: 노드 allocatable `nvidia.com/gpu: 1` 은 k8s 스케줄러의 것이었고, 0.0.96 은 k3s 클러스터 컨테이너를 버리고 docker 드라이버를 직접 쓰므로 사라짐 — GPU 샌드박스 2개 동시 Ready + 양쪽 `cuInit rc=0` 확인. 0.0.26 시절 원인 추적(가설 10건 전수 배제, landlock/seccomp/capability/netns/rlimit/mmap/mountinfo 비교)은 commit 6bb927c 에 남아 있다 — 이제 무의미하므로 여기서는 뺀다. **설계 단순화**: 하드링크 스테이징(`--stage`, `/hostfs` 컨테이너, EXDEV 회피)과 `kubectl patch` 로 Sandbox CR 에 hostPath·env·command 주입하는 단계가 **전부 삭제됨**. create 에 `--workdir` 가 없으므로 작업 디렉토리는 `-- sh -c "cd '$workdir' && exec $cmd"` 로 처리(CLI 가 argv 를 그대로 넘겨 따옴표·`$변수` 정상 동작, 실측), 환경변수는 `--env KEY=VALUE` 반복. **0.0.96 에서 새로 걸리는 것**: (a) 이미지에 OCI `USER` 필수 — 없으면 `OCI USER is required because run_as_user is omitted` 로 재시작 루프(Dockerfile.runtime 에 `USER sandbox` 추가함), (b) 살아있는 샌드박스에 `policy set` 으로 범위를 좁히는 건 거부됨(`read_write path '/dev/null' cannot be removed on a live sandbox`) — 정책은 create 시점에만 준다, (c) landlock 은 그대로라 마운트해도 정책에 없으면 `ls` 가 Permission denied — write_policy 로직 유지, (d) `forward start --background` 가 남기는 ssh 데몬이 스크립트 stdout 을 물려받아 파이프가 안 닫힘 → 리다이렉트로 차단, (e) 게이트웨이는 systemd **user** 서비스(`openshell-gateway.service`), CLI 등록은 `openshell gateway add https://127.0.0.1:17670 --local --name openshell`(`--local` 없으면 브라우저 인증 시도로 헤드리스에서 실패) | E2E 검증 완료(cosmos3nano): GPU 샌드박스에 venv + 33GB 웨이트를 bind mount(복사 0바이트, 스테이징 없음), `torch.cuda.is_available()=True`/`NVIDIA GB10`/matmul 성공, 웨이트 쓰기는 `Read-only file system` 으로 차단, `--down`→`--up` 왕복 5.4초. 2026-08-04 단계 전환 완료 — comfyui/t2i/kokoro/chatterbox/cosmos3nano **5종이 샌드박스에서 Ready**, 기존 포트 그대로 응답(:8188 `/system_stats`, :8501 `{"cuda":true,"device":"NVIDIA GB10"}`, :8503, :8504, :8505). GPU 샌드박스 5개 + CPU 1개 동시 상주. **함정**: comfyui 는 `comfyui.service`(systemd **user** 유닛)로 떠 있어서 kill 해도 systemd 가 즉시 되살려 :8188 을 다시 잡았고, forward 는 dead 인데 포트는 호스트 ComfyUI 가 200 을 응답해 전환이 성공처럼 보였다 — 포트 응답만으로 판정하면 안 된다. `systemctl --user stop/disable comfyui.service` 후 재전환하고 cgroup 으로 검증(실제 ComfyUI pid 가 `docker-...scope`, 샌드박스 내부 `exec` 로 프로세스·자기 서버 응답 확인). 스크립트에 반영: `--check` 는 포트를 쥔 systemd 유닛 이름과 stop/disable 명령을 출력하고, `--up` 은 `forward list` 가 running 인지 확인해 dead 면 실패시킨다. 전환 중 발견한 실제 버그: **읽기전용 venv 는 런타임에 site-packages 로 쓰는 라이브러리를 깨뜨린다** — chatterbox 의 librosa→numba 가 `cannot cache function '__o_fold': no locator available` 로 죽었고 `NUMBA_CACHE_DIR=/tmp` 로 해결(venv 를 rw 로 여는 게 아니라 캐시만 옮김). ollama 도 전환 완료(`sudo systemctl stop ollama` 후 `--up ollama`) — **6종 전부 샌드박스에서 Ready**, `curl 127.0.0.1:11434/api/tags` 가 모델 14개 응답. ollama 는 첫 기동에 `$HOME/.ollama` 로 키를 쓰는데 샌드박스 로그인 유저가 sandbox 라 HOME 이 `/home/sandbox` 였고 거긴 정책상 못 써서 죽었다(`could not create directory mkdir /home/sandbox/.ollama: permission denied`) — `HOME=/home/admin` 으로 돌려 rw 로 물린 `.ollama` 를 쓰게 해 해결. 샌드박스 밖 인터넷은 정책이 막으므로 `ollama pull` 은 별도 허용이 필요하다(로그: `Get "https://ollama.com:443/api/tags": Forbidden`). **주의**: forward 는 127.0.0.1 에 바인드한다. 전환 전 t2i(:8501)는 0.0.0.0 이었으므로 외부/컨테이너 접근은 끊긴다 — 필요하면 `forward start 0.0.0.0:<port>`. ollama 는 open-webui 가 `host.docker.internal:11434` 로 붙으므로 전환 시 이 바인드 문제를 먼저 확인할 것 | openshell sandbox list \| grep -q Ready && curl -sf 127.0.0.1:8505/health \| grep -q '"status":"ok"' | 8.2 | cc:완료 | - |
| 8.3 | README 갱신 (오픈셸 구성·기동법·국적표) [tdd:skip:docs-only] | README에 스튜디오 구성·기동·모델국적표 반영 | grep -qi openshell README.md | 8.2.1 | cc:완료 [1720268] | - |
