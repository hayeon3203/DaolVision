...
| 8.2.1 | 모델 서버 OpenShell 샌드박스 격리 (Ollama:11434/ComfyUI:8188/T2I:8501/Kokoro:8503/Chatterbox:8504/Cosmos3Nano:8505 총 6종 — Kokoro는 onnxruntime CPU판이라 `--gpu` 없이, 나머지 5종은 `--gpu` 샌드박스 + `forward`로 기존 localhost 포트 유지, tools.py 코드 변경 없음). **웨이트·venv·코드는 호스트에 그대로 두고 API 서버 프로세스만 격리** — openshell CLI엔 호스트 마운트 플래그가 없으나(2026-08-03 `sandbox create --help`/`gateway start --help`/`settings`/`policy` 전수 확인), 클러스터 컨테이너 `openshell-cluster-openshell`의 도커 볼륨이 호스트 `/var/lib/docker/volumes/openshell-cluster-openshell/_data` = 컨테이너 `/var/lib/rancher/k3s`로 양방향 노출되고 모델·볼륨이 동일 파일시스템(`/dev/nvme0n1p2` ext4)이라 **하드링크 스테이징(복사 0바이트)** 후 Sandbox CR에 hostPath 마운트로 주입. 서로 다른 bind mount 간 `ln`은 커널 `do_linkat`이 EXDEV로 막으므로 호스트 루트를 단일 마운트로 넣어야 함(`docker run -v /:/hostfs`). CR의 `podTemplate.spec`이 임의 volumes/volumeMounts 수용하는 것은 server dry-run으로 확인 — 단 CLI로 생성 후 `kubectl patch`해야 `openshell.ai/sandbox-id` 라벨이 남아 `openshell sandbox list`에 잡힘 | **(2026-08-03 실측 결과 GPU 서비스 5종 불가 — 아래 근거)** CPU 서비스 격리는 E2E 검증 완료: cosmos3nano를 샌드박스에서 기동해 `curl 127.0.0.1:8505/health` → `{"status":"ok","backend":"cosmos3-nano"}` 응답 확인(웨이트 33GB는 호스트 하드링크, 복사 0바이트, tools.py 무수정). **차단 사유 1 — supervisor 경유 시 CUDA 초기화 실패 (근본 원인 미특정)**: 같은 파드에서 `kubectl exec`(supervisor 우회)는 `cuInit rc=0`인데 `openshell sandbox exec`(supervisor 경유)는 `cuInit rc=304 (cudaErrorOperatingSystem)`. openshell 공식 GPU 이미지(`--from nvidia-gpu`)로도 동일(`nvidia-smi` → "Failed to initialize NVML: Insufficient Permissions"; 덤으로 그 이미지는 NVML 580.159 vs 호스트 드라이버 580.142 불일치). 하드웨어/이미지 문제 아님이 대조군으로 확인됨: 동일 이미지·동일 venv를 `docker run --gpus all`로 돌리면 `torch.cuda.is_available()=True, NVIDIA GB10`. **가설 검증 이력 — 아래 전부 실측으로 배제됨**: ① capability 박탈(supervisor는 `CapEff=0x0`, 우회는 `0x4a82c35fb`) — 그러나 `docker --cap-drop ALL`로도 `cuInit rc=0`이라 무관, ② 비루트 실행(`--user 1000:1000`) 무관, ③ `no-new-privileges` 무관, ④ seccomp가 ioctl 차단 — 샌드박스 안에서 `/dev/nvidiactl`·`/dev/nvidia0`·`/dev/nvidia-uvm` raw ioctl 직접 호출 시 양쪽 모두 동일하게 `EINVAL`/`ENOSYS`(드라이버 도달, EPERM 아님), ⑤ landlock 파일정책 — `strace`로 cuInit이 여는 경로 전수 추출(`/dev/nvidia*`, `/dev/shm`, `/proc/driver/nvidia/params`, `/sys/devices/system`, `/sys/bus/pci/devices`, `/usr/share/nvidia`, `$HOME/.nv`) 후 정확히 등록한 좁은 정책 `Status: Loaded` 확인해도 304, ⑥ 별도 netns — `docker --network none`으로도 `rc=0`, ⑦ rlimit/cgroup — MEMLOCK 8MB 등 양쪽 완전 동일, ⑧ mmap(익명·디바이스fd) 양쪽 동일. ⑨ `/dev` 마운트 구조 — `/proc/self/mountinfo` 양쪽 완전 동일, ⑩ `/dev` 디렉토리 리스팅 차단(supervisor 경유 시 `ls /dev` → Permission denied, 정책에 `- /dev` 를 read_only 로 추가해도 여전히 거부됨 — openshell 이 /dev 열거를 정책과 무관하게 막는 것으로 보임) — 그러나 호스트 strace 결과 libcuda 는 `/dev` 를 `getdents64` 로 열거하지 않고 디바이스 노드를 경로로 직접 open 하므로 인과 아님. supervisor 아래선 `strace`가 `PTRACE_SETOPTIONS: Operation not permitted`로 막혀 실패 지점 syscall 특정 불가. 커널 로그는 `adm` 그룹 권한으로 `journalctl -k` 확인 가능했으나 **실패 순간 NVRM/nvidia/LSM 관련 기록이 전무**(커널·감사 로그 완전 무음) — 즉 드라이버도 LSM 도 거부를 기록하지 않으며 실패는 libcuda 유저스페이스 내부에서 발생. **결론: 관측 가능한 모든 격리 차원이 동일한데 cuInit만 다름 → openshell supervisor 내부 프로세스 셋업 문제로 판단, upstream 이슈 후보.** **차단 사유 2 — GPU 1개만 노출**: 노드 allocatable `nvidia.com/gpu: 1`이라 GPU 샌드박스가 동시에 1개만 스케줄됨(2번째는 Pending). 5종 동시 격리가 구조적으로 불가. → GPU 5종 격리는 openshell 0.0.26에서 보류, 재도전은 upstream이 GPU 워크로드용 capability 예외를 제공한 뒤 | (보류) CPU 서비스 한정 — openshell sandbox list \| grep -q Ready && cd langgraph && ./.venv/bin/python driver.py --dry | 8.2 | cc:보류 | - |
