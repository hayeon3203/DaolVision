...
| Task | 내용 | DoD | Acceptance | Depends | Status | GH |
|------|------|-----|------------|---------|--------|----|
| 6.1 | S2 Flux Kontext 6스타일 (4.5 style_presets.py 재사용: cinematic/anime/cyberpunk/lowpoly/claymation/watercolor, 신규 정의 안 함) | 얼굴사진→6스타일 이미지 출력, :8700 /i2i 엔드포인트가 style_prefix() 그대로 호출 | curl -sf -X POST http://localhost:8700/i2i -F style=cinematic -F image=@test.jpg | grep -qi image | 4.1, 4.5 | cc:TODO | - |
